...
def parse_args():
    ...
    # parser.add_argument("--save_energy", default="", help="Output energy loss as json.")
...
if __name__ == "__main__":
	...
	if args.save_mesh:
		...
		if not os.path.exists(mesh_outpath):
			print('\nDirectory does not exists, will create: \n\t{}'.format(mesh_outpath))
			os.mkdir(mesh_outpath)
			print('\n......Directory created......'.format(mesh_outpath))
	...
	if True:
		if ref_transforms:
			...
			base_images = '/'.join(args.screenshot_transforms.split("/")[:-1])
			total_energy = 0.0
			testbed.fov_axis = 0
			...
			for idx in args.screenshot_frames:
				...
				if not os.path.splitext(outname)[1]:
					outname = outname + ".png"
				#print(base_images+f["file_path"])
				print(f"rendering {outname}")
				image = testbed.render(args.width or int(ref_transforms["w"]), args.height or int(ref_transforms["h"]), args.screenshot_spp, True)
				
				#create /rendered folder in '~/mini_mars_yard/scale_x_aabb_y' folder
				out_path = os.path.join(args.save_energy,'rendered') 
				if not os.path.exists(out_path):
					print('\nDirectory does not exists, will create: \n\t{}'.format(out_path))
					os.mkdir(out_path)
					print('\n......Directory created......'.format(out_path))

				#Save snapshots of mesh to compute energy 
				write_image(base_images+"/rendered/"+outname, image)
				image = cv2.imread(base_images+"/rendered/"+outname)
				actual_scan = cv2.imread(base_images+f["file_path"][1:])
				scale_factor = (ref_transforms["w"] + ref_transforms["h"])

				L1 = np.abs(image-actual_scan).sum()
				L2 = np.sum((image-actual_scan)**2)**.5

				energy = (L1+L2)/(2*scale_factor)
				total_energy += energy
				print("Energy:", energy, "Total Energy", total_energy)
	...
	data = {
		"scene" : base_images,
		"energy" : total_energy, 
		"time(sec)" : run_time_sec,
		"time(min)" : run_time_min
	}

	json_data = json.dumps(data, indent = 4) 

	if args.save_energy:
		file_dir = 	args.save_energy.split('/')[-1]
		file = str('energy_' + file_dir + '.json')

		json_file = os.path.join(args.save_energy, file)
		print('\nCreated file name from save_energy argument: {}'.format(json_file))

		# json_file = (base_images+"/energy.json")
		with open(json_file, 'w') as out:
			out.write(json_data)
			print('\nEnergy data successfully written to json file')
	else: 
		print('\nFAILED ERROR: File name from save_energy not created. Current Working Dir: {}'.format(os.getcwd()))
